...
def test_binary_operations(device, to_np, op_name, op_nano, op_torch):
    ...
    # --- nanograd ---
    a_nano = Tensor(np.array(data_a), label='a').to(device)
    b_nano = Tensor(np.array(data_b), label='b').to(device)
    ...
    np.testing.assert_allclose(to_np(res_nano), res_pt.detach().numpy(), rtol=1e-5, err_msg=f"Value error (forward) for: {op_name}")
    np.testing.assert_allclose(to_np(a_nano.grad), a_pt.grad.numpy(), rtol=1e-5, err_msg=f"Gradient error 'a' for: {op_name}")
    np.testing.assert_allclose(to_np(b_nano.grad), b_pt.grad.numpy(), rtol=1e-5, err_msg=f"Gradient error 'b' for: {op_name}")
...
def test_unary_operations(device, to_np, op_name, op_nano, op_torch):
    """Tests unary operations performed on a single Tensor."""
    
    data = [[-1.0, 2.0], [0.5, -0.5]]

    # --- nanograd ---
    x_nano = Tensor(np.array(data), label='x').to(device)
    ...
    # --- Assertions ---
    np.testing.assert_allclose(to_np(res_nano), res_pt.detach().numpy(), rtol=1e-5, err_msg=f"Value error (forward) for: {op_name}")
    np.testing.assert_allclose(to_np(x_nano.grad), x_pt.grad.numpy(), rtol=1e-5, err_msg=f"Gradient error for: {op_name}")
...
def test_complex_chain_rule(device, to_np):
    ...
    # --- nanograd ---
    x_nano = Tensor(np.array(data_x), label='x').to(device)
    y_nano = Tensor(np.array(data_y), label='y').to(device)
    ...
    # --- Assertions ---
    np.testing.assert_allclose(to_np(z_nano), z_pt.detach().numpy(), rtol=1e-5, err_msg="Value error in complex test")
    np.testing.assert_allclose(to_np(x_nano.grad), x_pt.grad.numpy(), rtol=1e-5, err_msg="Gradient error 'x' in complex test")
    np.testing.assert_allclose(to_np(y_nano.grad), y_pt.grad.numpy(), rtol=1e-5, err_msg="Gradient error 'y' in complex test")
...
def test_matmul(device, to_np):
    ...
    # --- nanograd ---
    a_nano = Tensor(np.array(data_a)).to(device)
    b_nano = Tensor(np.array(data_b)).to(device)
    ...
    # --- Assertions ---
    np.testing.assert_allclose(to_np(res_nano), res_pt.detach().numpy(), rtol=1e-5)
    np.testing.assert_allclose(to_np(a_nano.grad), a_pt.grad.numpy(), rtol=1e-5)
    np.testing.assert_allclose(to_np(b_nano.grad), b_pt.grad.numpy(), rtol=1e-5)
...
def test_scalar_operations(device, to_np):
    ...
    x_nano = Tensor(np.array(data_x)).to(device)
    res_nano = x_nano + 2.0
    res_nano.backward()
